...
import { FaVideo, FaImage, FaFilePdf, FaTrash, FaGoogle, FaEnvelope, FaDownload } from 'react-icons/fa';
...
const Upload = () => {
  ...
  const handleUpload = async () => {
    ...
    if (imageFile) {
      ...
      uploadedFileUrl = URL.createObjectURL(imageFile);
    }
    if (pdfFile) {
      formData.append('pdf', pdfFile);
      uploadedFileUrl = URL.createObjectURL(pdfFile);
    }
    ...
    try {
      ...
      // Stocker le lien exact dans MySQL
      await axios.post('http://localhost:5000/storeLink', {
        userId: user.$id,
        title,
        description,
        email,
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {uploads.map(upload => (
                <div key={upload.id} className="bg-white shadow-lg rounded-lg p-4">
                  ...
                  <button onClick={() => window.location.href = `mailto:${upload.email}`} className="bg-green-500 text-white px-4 py-2 rounded shadow mt-2">
                    <FaEnvelope /> Send Email
                  </button>
                  <button onClick={() => window.location.href = upload.imageUrl} className="bg-blue-500 text-white px-4 py-2 rounded shadow mt-2">
                    <FaDownload /> Download
                  </button>
                </div>
              ))}
